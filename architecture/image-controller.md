...
- **Cleanup**: When a Component CR is requested to be deleted, image controller will remove
  component's image repository and robot account from the remote registry
  ...
  ownership established between them.
...
based on `$DEFAULT_REGISTRY_ORG/$USER_NAMESPACE/$IMAGE_REPOSITORY_NAME`.
...
To regenerate tokens push (also pull when used with Component CR), set `true` to `.spec.credentials.regenerate-token`
...
- It will unlink secret from service account, if secret doesn't
  exist (you can recreate secret using 'regenerate-token').
