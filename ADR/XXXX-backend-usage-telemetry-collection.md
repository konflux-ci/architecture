...
The Segment API key would be installed in a secret in the `konflux-info` namespace that is readable both by the segment-bridge job and by the front-end UI code.
Similarly the Segment API URL will be stored in the `telemetry-api-url` key in the `cluster-info` config map in the `konflux-info` namespace.
