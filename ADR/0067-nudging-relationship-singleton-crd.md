...
- **`build-nudge-files`** (annotation on the push build PipelineRun,
  ...
  annotation-driven config. Users who rely solely on this annotation
  (without any ConfigMap-based configuration) must be explicitly
  notified of the deprecation via release notes and documentation
  updates, since the annotation has the lowest priority when ConfigMaps
  are present and these users may not be aware of the ConfigMap
  alternative.
