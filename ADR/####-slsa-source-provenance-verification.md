...
2. **Source provenance verification task** (in Konflux pipeline):
   ...
   - The task verifies the source VSA signature and contents
...
4. **Verification in Conforma**:
   ...
   - Through the chain, Conforma gains transitive trust in the source VSA without needing to directly verify signatures from external systems
...
- **Decouples source and build attestations**: Source VSAs remain in git notes (close to source), while our attestations remain in OCI registries (close to artifacts)
...
- **Storage model alignment**: Source attestations stored as git notes (close to source), build attestations about them stored in OCI (close to artifacts)
...
1. **New Tekton task**: Implement `verify-source` source provenance verification task that:
   ...
   - Verifies source VSA signatures and contents
