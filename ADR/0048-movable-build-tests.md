...
Each task will be responsible for generating an attestation in Cosign. `vuln`
attestations will be used for security scans rather than `provenance`
attestations. Tasks will output `ARTIFACT_URI` and `ARTIFACT_DIGEST` which will
point to the location of the attestation in Quay.  These results will tell
Chains to sign the attestation.
