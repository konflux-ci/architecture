...
* **Deployment:** The proxies will be deployed via a configurable Helm chart into a dedicated `proxy` namespace. A minimal, non-caching "passthrough" configuration will be part of the konflux-ci/konflux-ci "core" deployment to ensure systems are not prone to failure or security issues due to the proxy being unavalable.
