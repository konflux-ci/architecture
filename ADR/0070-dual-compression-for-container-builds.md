---
title: "70. Support for zstd:chunked image compression"
status: Proposed
applies_to:
  - build-service
topics:
...
* `push-rpm-data-to-pyxis` — uses `cosign download` to fetch SBOMs from child manifest digests, which works since each variant has its own SBOM with the correct digest as the main component
* `rpms-signature-scan` — crashes extracting zstd layers with `archive/tar: invalid tar header` because the RPM verifier tool does not understand zstd compression
...
* Provenance and signing. Because the build task explicitly exports the child manifests in `IMAGES`, Tekton Chains generates a single provenance attestation with all images (index, gzip child, zstd child) as subjects, avoiding trust gaps where manifests are only discovered by downstream tasks.
