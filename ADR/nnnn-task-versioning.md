...
Relates to [ADR NN. Trusted Tasks model after build-definitions decentralization](ADR/tbd.md).
...
in [ADR NN](ADR/tbd.md), the mechanism will not work after the build-definitions
decentralization.
...
For Tasks at version `0.x`, you may continue to mark breaking changes with minor
version updates and start marking non-breaking changes with patch version updates.
Do aim to bring the Task to version `1.0`.
...
Document the meaningful changes in each version in a CHANGELOG.md file.

### MintMaker: don't assume each x.y version has a MIGRATION.md

MintMaker links MIGRATION.md documents in the PRs it sends to Konflux users (see
the example [above](#users-get-opaque-updates)). This feature assumes that each
...
<!-- TODO: How will MintMaker figure out a link to the changelog? -->
MintMaker will stop trying to link MIGRATION.md files and instead link the changelog.
...
Extend the mechanism described in [ADR NN. Trusted Tasks model after build-definitions decentralization](ADR/tbd.md).
Give policy authors the ability to constrain the acceptable Task versions.
...
The [Appendix of ADR NN](ADR/tbd.md#appendix) illustrates how the Task version rules
could look.
...
> be trusted](ADR/tbd.md#decentralized-tasks-lose-the-trust-in-their-git-references),
