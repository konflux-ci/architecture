...
The IntegrationTestScenario pipelines in StoneSoup are not suitable for full-blown performance and scale testing (that usually takes lots of time and involves human analysis), but are a good place for a quick and small-scale regression test.
...
- Checking minimum and maximum of the metric in historical data and making sure the new result is in this range.
...
We are considering two different options here. They differ in what portion of logic mentioned above is executed where.
...
2. We gather configured monitoring metrics and add them to the JSON.
3. We upload the JSON with all the data to Horreum.
4. Horreum performs result analysis, looking for changes in configured metrics.
4. We get PASS/FAIL decision from Horreum back to the pipeline, so pipeline can rerurn proper result.
...
- Horreum is end-to-end solution that already exists, is already used by multiple teamsn and has documentation for the whole process.
