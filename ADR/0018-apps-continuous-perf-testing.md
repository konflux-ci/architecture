# 18. Continuous Performance Testing (CPT) of Apps in AppStudio
...
Goal of this ADR is to propose a way for this kind of testing to be implemented in AppStudio (feature [STONE-679](https://issues.redhat.com/browse/STONE-679)). Even if it would not deprecate full-blown performance and scale testing, having a release gate with some short and small scale performance test is desirable for many application teams.
...
Although Horreum provides rich web UI for configuring JSON parsing, change detection and data visualization, it will stay hidden to AppStudio users. AppStudio will expose subset of that functionality in it's own web UI and will talk to Horreum via it's API interface.
...
- AppStudio UI can display all the trends because historical data will be available in Horreum.
...
- Although Horreum uses Keycloak as well, some changes in the Horreum authentication mechanism might be required to cooperate with AppStudio member cluster RBAC.
...
- We would need to develop AppStudio UI to get graphs/results from Horreum and to allow users to configure the change detection parameters for their tests.
