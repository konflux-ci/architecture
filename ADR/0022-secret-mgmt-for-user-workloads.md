...
To distinguish secrets that were provided by end-users from secrets that are generated by RHTAP's components, a label `appstudio.redhat.com/internal: true` is introduced. The presence of this label indicates that the secret is intended for internal usage, and unauthorized changes to such secrets may lead to unpredictable results.
